 ...
 * @param {boolean} [opts.collapseTextNodes=false] - when true, elements that contain only
 * a single text or cdata child and have no attributes will be returned as a string
 * instead of an object. This is opt-in to preserve backward compatibility.
 * @returns {Object}
 */
function parse(xml, opts = {})
{
    ...
    const collapseTextNodes = opts.collapseTextNodes === true;
    ...
    function nodeToObject(node)
    {
        ...
        // If requested, collapse simple text/cdata-only nodes (with no attrs)
        if (collapseTextNodes && Object.keys(node.attrs).length === 0 && childrenArray.length === 1 && (childrenArray[0].type === 'text' || childrenArray[0].type === 'cdata'))
        {
            return childrenArray[0].text;
        }
